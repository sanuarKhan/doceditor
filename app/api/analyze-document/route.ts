...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log(`[NextAPI] Requesting text parsing from ${pdfServiceUrl}/parse...`);
        ...
        if (!parseResponse.ok) {
            ...
            throw new Error(`PDF Service Request Failed: ${parseResponse.status} ${parseResponse.statusText}`);
        }
        ...
        if (text && text.length > 50) {
            console.log(`[NextAPI] Text extracted (${text.length} chars). Analyzing with Gemini...`);
